...
import { getKEY, getLKEY } from '../lin/util/storage'
let stageMap = {}
...
export const user = state => state.user // 登陆者信息

export const serviceList = state => state.serviceList // 客服列表

export const readedMessages = state => state.readedMessages

export const unreadMessages = state => state.unreadMessages

export const QCallUser = state => state.callUserInfo// 来电用户信息

export const vistJumpSheetCreateInfo = state =>state.vistJumpSheetCreateInfo || getKEY('vistJumpSheetCreateInfo')
...
export const sideBarList = (state, getter) => {
  ...
  // 判断如果为生产环境，加上service前缀
  const data = permissionStageConfig.map(item => {
    if(process.env.NODE_ENV === 'production') {
      if (filterIcon(item.icon)) {
        item.icon = '/service' + item.icon
      }
    }
    return item
  })
  ...
  const sideBar = deepGetSideBar(data, sideBarLevel)
  return sideBar
}

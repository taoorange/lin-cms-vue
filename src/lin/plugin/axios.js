...
_axios.interceptors.request.use(
  originConfig => {
    ...
    // 设置接口请求头
    // if (reqConfig.cType === 2) {
    //   reqConfig.headers = { 'Content-Type': CONTENT_TYPES[reqConfig.cType] }
    // } else if (reqConfig.cType === 3) {
    //   reqConfig.data = qs.stringify(options.data)
    //   reqConfig.headers = { 'Content-Type': CONTENT_TYPES[reqConfig.cType] }
    // }
    ...
    } else if (reqConfig.method === 'post' || reqConfig.method === 'patch') {
      if (!reqConfig.data) {
        // 防止字段用错
    ...
    // step2: permission 处理
    if (reqConfig.url === '/v1/users/refresh') {
  ...
  async res => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (code === 10041 || code === 10051) {
        ...
        if (cache.url !== url) {
          cache.url = url
          const responseToken = await _axios({
            method: 'get',
            url:'/v1/users/refresh'
          })
          if(responseToken.code === 11) {
            saveAccessToken(responseToken.data.access_token)
            saveTokens(responseToken.data.access_token, responseToken.data.refresh_token)
          }
          // 将上次失败请求重发
